function [xopt, fopt, exitflag, output] = OptimizeTruss()
    ...
    function [f, c, ceq, memberStresses] = objcon(x)
        %---Design Variables---%   
        % Angle in degrees
        theta1 = x(1);             
        % Length in meters
        ...
        [numberOfMembers,~] = size(memberStresses);
        ...
        for index = 1:numberOfMembers
            c(2*index-1) = (memberStresses(index)-maxCompressiveForce);
            c(2*index) = maxTensionForce-memberStresses(index);
        end 
